...
function MM_swapImage() { //v3.0
  ...
  function dealerLogin() {
	...
	if ($('#dealerLoginEmail').val().length || $('#dealerLoginPassword').val().length) {
		...
		$.get('_ajax/dealer_login.cfm',{rand:Math.random(),email:$('#dealerLoginEmail').val(),password:$('#dealerLoginPassword').val()},function(data) {
			var arrResult = data.split('|');																															   			var result = parseInt(arrResult[0]);
			if (result==1) {
				if (arrResult.length == 1) {
  ...
  function passwordReminder() {
		...
		if (!$('#passwordReminderEmail').val().length) {
			...
			$.get('_ajax/send_password_reminder.html',{rand:Math.random(),email:$('#passwordReminderEmail').val()},function(data) {
				if (parseInt(data)==1) {
  ...
  function dealerPasswordReminder() {
		...
		if (!$('#dealerPasswordReminderEmail').val().length) {
			...
			$.get('_ajax/send_clientpassword_reminder.cfm',{rand:Math.random(),email:$('#dealerPasswordReminderEmail').val()},function(data) {
				if (parseInt(data)==1) {
  ...
  function isValidEmail(email) {
	...
	function addProduct(event) {
		...
		$.getJSON('_ajax/basket.cfm',{rand:Math.random(),product:id,colour:colourID},function(data) {
		...
		if (errArr.length) {
			...
			$.get('_ajax/check_email.cfm',{rand:Math.random(),email:$('#registerEmail').val()},function(data) {
				if (parseInt(data) != 0) {
					...
					$.get('_ajax/register_user.html',{rand:Math.random(),firstname:$('#registerFirstname').val(),lastname:$('#registerLastname').val(),email:$('#registerEmail').val(),password:$('#registerPassword').val(),gender:gender},function(data) {
	...
	function dealerRegister() {
		...
		if (errArr.length) {
			...
			$.get('_ajax/check_dealer_email.cfm',{rand:Math.random(),email:$('#dealerRegisterEmail').val()},function(data) {
				if (parseInt(data) != 0) {
					alert('Apologies, but this email address is already registered by a user.\n\nIf this is your email address, please try logging in with your registered details.');	
				} else {
					$.get('_ajax/register_dealer.cfm',{rand:Math.random(),dealer:$('#dealerRegisterDealer').val(),name:$('#dealerRegisterName').val(),email:$('#dealerRegisterEmail').val(),telephone:$('#dealerRegisterTelephone').val(),address:$('#dealerRegisterAddress').val(),website:$('#dealerRegisterWebsite').val()},function(data) {
	...
	function refreshBasket(showBasket) {
		$.getJSON('_ajax/basket_get.cfm',{rand:new Date().getTime()},function(data) {
	...
	$(document).ready(function() {
		...
		$('#facebookLink').attr({'href':globals.page.facebookLink,'target':'_blank'});
		$('#twitterLink').attr({'href':globals.page.twitterLink,'target':'_blank'});
		...
		$('#passwordReminderLink,#clientPasswordReminderLink,#pressPasswordReminderLink,#dealerPasswordReminderLink').fancybox({titleShow:false,overlayOpacity:0});
		$('.registrationLink,#clientRegistrationLink,#clientLoginLink,#pressRegistrationLink,#pressLoginLink,#dealerRegistrationLink,#dealerLoginLink').fancybox({titleShow:false,overlayOpacity:0});
		$('#registerCompleteLink,#clientRegisterCompleteLink,#pressRegisterCompleteLink,#dealerRegisterCompleteLink').fancybox({titleShow:false,overlayOpacity:0});
