...
function handleColourChangeClick(event) {
    ...
    switch(changeArea) {
        case 'secondHand':
            $svg.find('#secondHand path').attr({'fill':'#'+newColour});
            watches.currentWatch.secondHand = classVal;
            break;
        case 'luminescentIndices':
            $svg.find('#indices path,#indices_3_ path').attr({'fill':'#'+newColour});
            $svg.find('#bigHourHand,#bigMinuteHand,#littleHourHand,#littleMinuteHand').attr({'style':'fill:#'+newColour});
            watches.currentWatch.indices = classVal;
            break;
        case 'minuteMarkers':
            $svg.find('#fiveMinuteMarkers path,#fiveMinuteMarkers_1_ path').attr({'fill':'#'+newColour});
            ...
            break;
        case 'daytonaText':
            $svg.find('#daytonaText path,#daytonaText_2_ path').attr({'fill':'#'+newColour});
            watches.currentWatch.daytonaText = classVal;
            break;
        case 'chronoDials':
            $svg.find('#chronoDials circle,#chronoDials_1_ circle').attr({'fill':'#'+newColour});
            watches.currentWatch.chronoDials = classVal;
            break;
        case 'chronoRings':
            $svg.find('#chronoRings path,#chronoRings_1_ path').attr({'fill':'#'+newColour});
            watches.currentWatch.chronoRings = classVal;
            break;
        case 'chronoHands':
            $svg.find('#chronoHands path,#chronoHands_1_ path').attr({'fill':'#'+newColour});
            watches.currentWatch.chronoHands = classVal;
            break;
        case 'chronoMarkers':
            $svg.find('#chronoMarkers,#chronoMarkers_2_').attr({'fill':'#'+newColour});
            watches.currentWatch.chronoMarkers = classVal;
            break;
        case 'secondMarkers':
            $svg.find('#secondMarkers_2_,#secondMarkers').attr({'fill':'#'+newColour});
            watches.currentWatch.secondMarkers = classVal;
            break;
        case 'dial':
            $svg.find('#dial,#dial_3_').attr({'fill':'#'+newColour});
            watches.currentWatch.dial = classVal;
            break;
        case 'logo':
            $svg.find('#logo path,#logo_6_ path').attr({'fill':'#'+newColour});
            ...
            break;
        case 'dialText':
            $svg.find('#dialText_4_,#dialText_1_').parent().attr({'fill':'#'+newColour});
...
function populateWatch() {
    ...
    $svg.find('#indices path,#indices_3_ path').attr({'fill':'#'+colour});
    $svg.find('#bigHourHand,#bigMinuteHand,#littleHourHand,#littleMinuteHand').attr({'style':'fill:#'+colour});
    // DAYTONA TEXT
    ...
    $svg.find('#daytonaText path,#daytonaText_2_ path').attr({'fill':'#'+colour});
    // second Markers
    ...
    $svg.find('#secondMarkers_2_,#secondMarkers').attr({'fill':'#'+colour});
    // minute Markers
    ...
    $svg.find('#fiveMinuteMarkers path,#fiveMinuteMarkers_1_ path').attr({'fill':'#'+colour});
    ...
    $svg.find('#dial_3_,#dial').attr({'fill':'#'+colour});
    // second hand
    ...
    $svg.find('#secondHand path').first().attr({'fill':'#'+colour});
    // logo
    ...
    $svg.find('#logo path,#logo_6_ path').attr({'fill':'#'+colour});
    ...
    $svg.find('#chronoRings path,#chronoRings_1_ path').attr({'fill':'#'+colour});
    // chronoDials
    ...
    $svg.find('#chronoDials circle,#chronoDials_1_ circle').attr({'fill':'#'+colour});
    // chronoHands
    ...
    $svg.find('#chronoHands path,#chronoHands_1_ path').attr({'fill':'#'+colour});
    // chronoMarkers
    ...
    $svg.find('#chronoMarkers,#chronoMarkers_2_').attr({'fill':'#'+colour});
    // dial text
    ...
    $svg.find('#dialText_4_,#dialText_1_').parent().attr({'fill':'#'+colour});
    if (watches.currentWatch.dial == watches.defaultWatch.dial && watches.currentWatch.chronoRings == watches.defaultWatch.chronoRings && watches.currentWatch.chronoHands == watches.defaultWatch.chronoHands && watches.currentWatch.chronoDial == watches.defaultWatch.chronoDial && watches.currentWatch.chronoMarkers == watches.defaultWatch.chronoMarkers) {
        $svg.find('#chronoRingsMetallic,#chronoRingsMetallic_1_').show();
    } else {
        $svg.find('#chronoRingsMetallic,#chronoRingsMetallic_1_').hide();
...
function postToFacebook() {
    FB.login(function(response) {
        if (response.status==='connected') {
            $('#fbWaitIndicator').animate({'left':'0px'},600,'easeInOutExpo',function() {
                $.ajax({
                    type: "POST",
                    ...
                    success: function(data){
                        $('#fbThanks').animate({'left':'0px'},600,'easeInOutExpo');
                    }
                });
            });
        }
